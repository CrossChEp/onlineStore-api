...
from models import add_product_to_user_bag, get_products_from_user_bag, delete_product_from_user_bag
...
@bag_router.delete('/api/bag/{product_id}')
def delete_product_from_bag(product_id: int, user: User = Depends(get_current_user),
                            session: Session = Depends(generate_session)) -> None:
    delete_product_from_user_bag(product_id=product_id, user=user, session=session)
